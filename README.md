...
Important practical notes (what to be aware of)

The script uses historical daily returns and assumes a 252 trading-day year to annualize returns.

No short selling is allowed (weights constrained 0–1) and weights are fully invested (sum = 1).

The optimizer uses numerical methods and may produce concentrated portfolios (one or few large weights) depending on the data and the chosen criterion — this is expected behavior in unconstrained mean-variance type optimization.

The SK_criterion and higher-moment objectives (skewness/kurtosis) can be unstable with limited data — results should be interpreted with that in mind.

To reproduce results you need internet access (for yfinance), the listed Python packages installed, and enough history for each ticker.
